...
var updateScripts = function() {
    var promise = new Promise((resolve, reject) => {
        gitPullOrClone(process.env.RPSHOME,process.env.RPS_GIT).
        then((res) => {
            if(process.env.INCLUDE_EXTRA)
            {
                var result = [res];
                gitPullOrClone('/RetroPie-Extra',process.env.RP_EXTRA_GIT).
                then(res2 => {
                    result.push(res2);
            ...
            logger.error("GitUtil","updateScripts","Update Scripts failed", err);
            reject([err]);
        });
    });
    return promise;
