...
var checkDir = function(dir) {
    var promise = new Promise((resolve,reject) => {
        logger.debug('Monitor','checkDir','before',dir);
        fs.exists(dir).then(exist => {
            ...
            } else
            {
                fs.realpath(dir).then(result => { 
                    logger.debug('Monitor','checkDir','after',result);
                    resolve(result) 
                });
            }
        });
        ...
        checkDir(directory).then(dir => {
            ...
            } else
            {
                find.file(new RegExp(expression),dir,(files) => {
                    ...
                    logger.debug("Monitor","searchFiles",directory,dir,expression)
                    resolve(results);
                })
            }
        })
...
var getLogs = function()
{
    var promise = new Promise((resolve,reject) => {
        var promises = [];
        logger.debug('Monitor','getLogs',"/dev/shm","runcommand(.*)");
        logger.debug('Monitor','getLogs',process.env.EMULATION_HOME,"(.*)log(.*)");
        logger.debug('Monitor','getLogs',"/root/.pm2/logs/","RetroPie(.*)");
        promises.push(searchFiles("/dev/shm","runcommand(.*)"));
        promises.push(searchFiles(process.env.EMULATION_HOME,"(.*)log(.*)"))
...
var getScreenShot = function(params,res)  {
    var promise = new Promise((resolve,reject) => {
        var args = ['-s'];
        if(params.height)
        {
            args.push('-h');args.push(params.height);
        }
        if(params.width)
        {
            args.push('-w');args.push(params.width);
        }
        
        logger.debug('Monitor','getScreenshot',args);
        var child = spawn('raspi2png',args);
