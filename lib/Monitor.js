...
{
    if(app.cmd.toUpperCase().search('RETRO') >0 && app.name != 'sh' && app.cmd.search('/bin/sh'))
    {
...
var getPIDs = function(data)
{
    var promise = new Promise((resolve,reject) => {
        psList().then(psds => {
            var myApps = psds.filter(appFilter).map(pd => {pd.name = pd.cmd.substr(pd.cmd.lastIndexOf('/')+1);return pd});
            if(data != null)
            {
                data.pids = myApps;
            } else
