...
var checkInstalled = function() {
    logger.debug("RetroPieHelper", "checkInstalled","Starting");
 ...
 **/
var init = function() {
    logger.debug('RetroPieHelper','Starting init');
    var promise = new Promise(function(resolve, reject) {
        if(process.env.RETROARCH_HOME == "" || !fs.existsSync(path.join(process.env.RPSHOME))) {
            logger.debug('RetroPieHelper','Checking installed');
            ...
            logger.debug('RetroPieHelper','process.env.RETROARCH_HOME',process.env.RETROARCH_HOME);
            resolve();
        }
    });
    return promise;
