...
var init = function() {
    ...
    var promise = new Promise((resolve, reject) => {
        fs.exists(path.join(process.env.RPSHOME, "retropie_packages.sh")).then(exist => { 
            if(exist) {
            ...
            {
                logger.debug("ShellUtil", "init", "Exiting");
                reject("RetroPie-Setup is not cloned yet");
            }
        })
    });
    return promise
...
var parseFiles = function() {
    var promise = new Promise(function(resolve, reject) {
        init().then(readScriptModules.bind(null)).then((results) => { resolve(results) }).catch(err => {
            reject();
        });
    })
    return promise;
