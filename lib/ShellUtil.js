...
var scriptModules = process.env.RPSHOME + path.sep + "scriptmodules"
/**************************************
 *
...
var parseFiles = function() {
    var promise = new Promise(function(resolve, reject) {
        init().then(function() {
            logger.debug("ShellUtil","parseFiles","starting");
            fs.readdir(scriptModules, (err, folders) => {
                if (err) {
                    logger.warn("ShellUtil", "parseFiles", "Failure:", err)
                }
                            ...
                            files.forEach(file => {
                                if (fs.lstatSync(path.join(scriptModules, folder, file)).isFile() && file.search(".sh") > 0) {
                                    ...
                                    rl.on('line', function(line) {
                                        ...
                                        } else if (app.gitURL == "" && line.search('gitPullOrClone') != -1) {
                                            var url = line.trim().split(' ')[2];
                                            if(url.search('https') >= 0 && url.search("\\$") == -1) {
                                                url = url.replace('.git','').replace(/"/g,"");
                                                app.gitURL = url;
                                                app.sites.push(url);
                                            }
                                        }
