...
var createCommand = function(name,command)
{
    ...
    var promise = new Promise((resolve,reject) => {
        ...
        var f = path.join(process.env.RPSHOME,'logs',name+"_"+command+".log");
        fs.mkdirsSync(path.join(process.env.RPSHOME,'logs'));
        var out = fs.openSync(f,'a');
        var child = spawn(file,[name,command],{
...
var execute = function(exec) {
    var promise = new Promise((resolve,reject) => {
        ...
        } else
        {
            ...
            que.add(createCommand.bind(null,exec.name,command)).then(result => {
                if(!exec.isFull)
                {
