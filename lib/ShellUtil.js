...
//The location where the application shell scripts are stored.
var scriptModules = path.join(process.env.RPSHOME,"scriptmodules");
var home = path.join(path.sep,'home',process.env.SUDO_USER);
var datadir = path.join(home,'RetroPie');
var biosdir = path.join(datadir,'BIOS');
var romdir = path.join(datadir,'roms');
var emudir = "";
var configdir = "";
...
var runNext = function() {
    if (que.pendingPromises == 0) {
        ...
        DBUtil.getLatestExec('sh').then(result => {
            if (result != null) {
                logger.debug('ShellUtils', 'runNext', '\tExecution Found');
                ...
                DBUtil.updateExec(exec).then(() => {
                    ...
                    execute(exec).then(() => {
                        logger.debug('ShellUtils', 'runNext', '\tFinished cron item', exec.name, exec.command)
                    }).catch((err) => {
                        logger.warn('ShellUtils', 'runNext', '\tFailed cron item', exec.name, exec.command)
                    });
                });
            }
        }).catch(err => {
            logger.warn('ShellUtils', 'runNext', 'Failed to get the latest Exec', err);
        });
    }
}
 ...
 **/
var init = function() {
    logger.debug("ShellUtil", "init", "Starting");
 ...
 * TODO Three is a bug when running fullIntall where the result has a result object.
...
var readFile = function(folder,file)
{
    var promise = new Promise((resolve,reject) => {
        fs.readFile(path.join(scriptModules, folder, file)).then(result => {
            ...
            app.help = app.help.replace(/\$home/g,home);
            app.help = app.help.replace(/\$datadir/g,datadir);
            app.help = app.help.replace(/\$biosdir/g,biosdir);
            app.help = app.help.replace(/\$romdir/g,romdir);
            app.help = app.help.replace(/\$emudir/g,emudir);
            app.help = app.help.replace(/\$configdir/g,configdir);
            delete app['id'];
            delete app['section'];
            delete app['desc'];
            if(app.name != null)
